<template>
  <div class="px-4 md:px-6 lg:px-8 mx-auto max-w-7xl py-8">
    ...
    <div v-else-if="reservations.length === 0" class="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
      ...
      <div class="mt-6">
        <NuxtLink to="/facilities" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-primary-600 hover:bg-primary-700">Przeglądaj obiekty</NuxtLink>
      </div>
    </div>

    <div v-else class="space-y-4">
      <div v-for="reservation in reservations" :key="reservation.id" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
        ...
        <div v-if="reservation.status === 'PENDING' || reservation.status === 'CONFIRMED'" class="mt-4 flex gap-2">
          <button @click="cancelReservation(reservation.id)" :disabled="cancellingId === reservation.id" class="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 text-sm font-medium disabled:opacity-50">
...
interface Reservation {
  id: string; facilityId: string; userId: string; teamId?: string
  date: string; startTime: string; endTime: string
  status: string; totalPrice: number; notes?: string
  facility?: { name: string; address: string; city: string }
  team?: { name: string }
}
interface ReservationsResponse { data: Reservation[] }
...
const formatDate = (date: string): string => {
  return new Date(date).toLocaleDateString('pl-PL', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })
}
</script>