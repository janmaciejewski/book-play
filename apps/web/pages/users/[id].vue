<template>
  <div class="px-4 md:px-6 lg:px-8 mx-auto max-w-7xl py-8">
    ...
    <AppError v-else-if="error" message="Nie udało się załadować profilu." />
    <div v-else-if="user">
        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
          <div class="flex items-center gap-4">
            <div class="relative flex-shrink-0">
              <UserAvatar :first-name="user.first_name" :last-name="user.last_name" size="lg" :avatar="user.avatar" />
...
interface UserData {
  ...
  avatar?: string; role: string; bio?: string; city?: string; country?: string
  position?: string; preferred_position?: string; age?: number; is_active: boolean
}
...
watch(showEditModal, (s) => {
  if (s && user.value) {
    editProfile.phone = user.value.phone || ''
    editProfile.city = user.value.city || ''
    editProfile.country = user.value.country || ''
    editProfile.age = user.value.age || null
    editProfile.position = user.value.position || ''
    editProfile.preferred_position = user.value.preferred_position || ''
    editProfile.bio = user.value.bio || ''
  }
})
...
async function saveProfile() {
  saving.value = true
  try {
    const r = await fetch(`${apiBase}/users/${userId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${useCookie('token').value || ''}` }, body: JSON.stringify({ phone: editProfile.phone || null, city: editProfile.city || null, country: editProfile.country || null, age: editProfile.age || null, position: editProfile.position || null, preferred_position: editProfile.preferred_position || null, bio: editProfile.bio || null }) })
    if (r.ok) { showToast('Zaktualizowano', 'success'); showEditModal.value = false; refresh() }
...
async function handleAvatarUpload(e: Event) {
  ...
  try {
    ...
    const r = await fetch(`${apiBase}/users/${userId}/avatar`, { method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: formData })
    if (r.ok) { showToast('Awatar zaktualizowany', 'success'); refresh() }
    else { const e = await r.json(); showToast(e.error || 'Błąd', 'error') }
  } catch { showToast('Błąd połączenia', 'error') }
}
</script>