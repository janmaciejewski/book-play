      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 mb-6">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center"><span class="text-2xl font-bold text-primary-600 dark:text-primary-400">{{ user.first_name?.[0] }}{{ user.last_name?.[0] }}</span></div>
            <div>
            ...
            <div class="ml-auto"><NuxtLink to="/teams" class="text-primary-600 dark:text-primary-400 hover:underline text-sm">&larr; Powrót</NuxtLink></div>
          </div>
        </div>
        <div class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div><h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase mb-1">Pozycja</h3><p class="text-gray-900 dark:text-white">{{ user.position||'—' }}</p></div>
    ...
    <div v-if="showEditModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form @submit.prevent="saveProfile" class="space-y-4">
          <div><label class="block text-sm font-medium mb-1">Telefon</label><input v-model="editProfile.phone" class="w-full rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500" /></div>
          <div><label class="block text-sm font-medium mb-1">Miasto</label><input v-model="editProfile.city" class="w-full rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500" /></div>
          <div><label class="block text-sm font-medium mb-1">Kraj</label><input v-model="editProfile.country" class="w-full rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500" /></div>
          <div><label class="block text-sm font-medium mb-1">Wiek</label><input v-model.number="editProfile.age" type="number" min="0" max="120" class="w-full rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500" /></div>
          <div><label class="block text-sm font-medium mb-1">Pozycja</label><input v-model="editProfile.position" placeholder="Napastnik, Obrońca..." class="w-full rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500" /></div>
          <div><label class="block text-sm font-medium mb-1">Preferowana pozycja</label><input v-model="editProfile.preferred_position" class="w-full rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500" /></div>
          <div><label class="block text-sm font-medium mb-1">O mnie</label><textarea v-model="editProfile.bio" rows="4" class="w-full rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Kilka słów o sobie..."></textarea></div>
...
interface UserData { id:string;email:string;first_name:string;last_name:string;phone?:string;role:string;bio?:string;city?:string;country?:string;position?:string;preferred_position?:string;age?:number;is_active:boolean }
...
const showEditModal=ref(false); const saving=ref(false)
