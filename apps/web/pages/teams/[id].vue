    ...
    <div v-else-if="team">
            ...
            <div v-else class="divide-y divide-gray-200 dark:divide-gray-700">
              <div v-for="member in members" :key="member.id" class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50">
                <div class="flex items-center">
                  <div class="w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center"><span class="text-primary-600 dark:text-primary-400 font-medium">{{ member.user?.firstName?.[0] }}{{ member.user?.lastName?.[0] }}</span></div>
                  <div class="ml-4">
      ...
      <div v-if="showAddMemberModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
          <h2 class="text-xl font-bold mb-4">Dodaj członka</h2>
      ...
      <div v-if="showRemoveConfirm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-sm"><h2 class="text-lg font-bold mb-2">Usuń członka</h2><p class="text-gray-600 dark:text-gray-400 mb-4">Usunąć <strong>{{ memberToRemove?.user?.firstName }} {{ memberToRemove?.user?.lastName }}</strong>?</p>
          <div class="flex gap-2 justify-end"><button @click="showRemoveConfirm=false;memberToRemove=null" class="px-4 py-2 border rounded-md">Anuluj</button><button @click="removeMember" :disabled="removingMember" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 disabled:opacity-50">{{ removingMember?'Usuwanie...':'Usuń' }}</button></div>
        </div>
      </div>
