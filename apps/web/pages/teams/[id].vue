<template>
  <div class="px-4 md:px-6 lg:px-8 mx-auto max-w-7xl py-8">
    ...
    <AppError v-else-if="error" message="Nie udało się załadować drużyny." />
    <div v-else-if="team">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start gap-4">
          <div class="flex items-center gap-4">
            <div class="relative flex-shrink-0">
              <UserAvatar :first-name="team.name" size="lg" :avatar="team.logo ? getLogoUrl(team.logo) : null" />
              <button v-if="canManage" @click="triggerLogoUpload" class="absolute -bottom-1 -right-1 bg-primary-600 text-white rounded-full p-1 hover:bg-primary-700" title="Zmień logo"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
            ...
            <div>
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <button v-if="canManage" @click="toggleEditInfo" class="text-gray-400 hover:text-primary-600 dark:hover:text-primary-400" title="Edytuj"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
              </div>
              <div v-if="editingInfo" class="mt-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border">
                <div class="mb-3"><label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Nazwa</label><input v-model="editForm.name" class="w-full rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500" /></div>
                <div class="mb-3"><label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Opis</label><textarea v-model="editForm.description" rows="3" class="w-full rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500"></textarea></div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2">
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow border">
            ...
            <div v-else class="divide-y divide-gray-200 dark:divide-gray-700">
              <div v-for="member in members" :key="member.id" class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50">
                <div class="flex items-center">
                  <UserAvatar :first-name="member.user?.firstName" :last-name="member.user?.lastName" size="sm" />
                  <div class="ml-4">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <StatusBadge v-else :status="member.role" size="sm" />
          ...
          <div v-if="myMembership || authStore.user?.role === 'ADMIN'" class="bg-white dark:bg-gray-800 rounded-lg shadow border mt-6">
            <div class="px-6 py-4 border-b"><h2 class="text-xl font-semibold text-gray-900 dark:text-white">Czat drużyny</h2></div>
            <div ref="chatMessagesEl" class="px-6 py-4 h-64 overflow-y-auto space-y-3">
              <div v-if="chatMessages.length === 0" class="text-center text-gray-500 dark:text-gray-400 py-8">Brak wiadomości. Rozpocznij rozmowę!</div>
            ...
            <div class="px-6 py-3 border-t border-gray-200 dark:border-gray-700 flex gap-2">
              <input v-model="newMessage" type="text" class="flex-1 rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm" placeholder="Napisz wiadomość..." @keyup.enter="sendChatMessage" />
              <button @click="sendChatMessage" :disabled="!newMessage.trim() || sendingChat" class="px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700 disabled:opacity-50 text-sm">{{ sendingChat ? '...' : 'Wyślij' }}</button>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border">
            ...
            <div class="space-y-3 text-sm">
              <div><span class="text-gray-500 dark:text-gray-400">Twój status:</span>
                ...
                <StatusBadge v-else :status="myMembership?.role || ''" size="sm" />
              </div>
          ...
          <div v-if="myMembership" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border">
          ...
          <div v-if="canManage" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border">
            <h3 class="font-semibold text-gray-900 dark:text-white mb-3">Rekrutacja</h3>
            <div class="flex items-center justify-between mb-3"><span class="text-sm text-gray-600 dark:text-gray-400">Otwarta</span><button @click="toggleRecruitment" :disabled="togglingRecruitment" :class="['relative inline-flex h-6 w-11 items-center rounded-full transition-colors', team.recruitment_open ? 'bg-green-600' : 'bg-gray-300 dark:bg-gray-600']"><span :class="['inline-block h-4 w-4 rounded-full bg-white transition-transform', team.recruitment_open ? 'translate-x-6' : 'translate-x-1']" /></button></div>
            ...
            <div v-if="applications.length > 0" class="space-y-3 max-h-60 overflow-y-auto">
              <div v-for="app in applications" :key="app.id" class="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg border">
                <div class="flex items-center justify-between mb-1"><p class="text-sm font-medium">{{ app.user?.first_name }} {{ app.user?.last_name }}</p><StatusBadge :status="app.status" size="sm" /></div>
      ...
      <div v-if="showAddMemberModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="showAddMemberModal = false">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
          ...
          <div class="mb-4"><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Rola</label><select v-model="addMemberRole" class="w-full rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white"><option value="MEMBER">Członek</option><option value="CAPTAIN">Kapitan</option></select></div>
      ...
      <!-- Forgot Password Modals - could these be refactored too? Not today. -->
      <div v-if="showRemoveConfirm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="showRemoveConfirm = false">
        <ConfirmDialog :title="'Usuń członka'" :message="`Usunąć ${memberToRemove?.user?.firstName} ${memberToRemove?.user?.lastName}?`" confirm-text="Usuń" :loading="removingMember" loading-text="Usuwanie..." @confirm="removeMember" @cancel="showRemoveConfirm = false; memberToRemove = null" />
      </div>
...
const canManage = computed(() => myMembership.value?.role === 'CAPTAIN')
...
async function saveInfo() {
  saving.value = true
  try {
    const r = await fetch(`${apiBase}/teams/${teamId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${useCookie('token').value || ''}` }, body: JSON.stringify({ name: editForm.name, description: editForm.description || null }) })
    if (r.ok) { editingInfo.value = false; showToast('Zaktualizowano', 'success'); refresh() }
...
async function handleLogoUpload(e: Event) {
  ...
  const file = input.files[0]; if (!['image/jpeg', 'image/png', 'image/webp', 'image/svg+xml'].includes(file.type)) { showToast('Nieobsługiwany format', 'error'); return }
  if (file.size > 5 * 1024 * 1024) { showToast('Zbyt duży plik (max 5MB)', 'error'); return }
  const fd = new FormData(); fd.append('logo', file)
  try {
    const r = await fetch(`${apiBase}/teams/${teamId}/logo`, { method: 'POST', headers: { Authorization: `Bearer ${useCookie('token').value || ''}` }, body: fd })
    if (r.ok) { showToast('Logo zaktualizowane', 'success'); refresh() }
    else { const e = await r.json(); showToast(e.error || 'Błąd', 'error') }
  } catch { showToast('Błąd połączenia', 'error') }
  input.value = ''
}
...
const showAddMemberModal = ref(false); const searchQuery = ref(''); const searchResults = ref<User[]>([]); const selectedUser = ref<User | null>(null); const addMemberRole = ref('MEMBER'); const addingMember = ref(false)
let searchTimeout: ReturnType<typeof setTimeout> | null = null
async function searchUsers() {
  if (searchTimeout) clearTimeout(searchTimeout)
  if (searchQuery.value.length < 2) { searchResults.value = []; return }
  searchTimeout = setTimeout(async () => {
    try { const r = await fetch(`${apiBase}/teams/${teamId}/search-users?q=${encodeURIComponent(searchQuery.value)}`, { headers: { Authorization: `Bearer ${useCookie('token').value || ''}` } }); if (r.ok) { const d: { data: User[] } = await r.json(); searchResults.value = d.data || [] } } catch { }
  }, 400)
}
function selectUser(user: User) { selectedUser.value = user; searchQuery.value = user.email; searchResults.value = [] }
async function addMember() {
  if (!selectedUser.value) return; addingMember.value = true
  try {
    const r = await fetch(`${apiBase}/teams/${teamId}/members`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${useCookie('token').value || ''}` }, body: JSON.stringify({ email: selectedUser.value.email, role: addMemberRole.value }) })
    if (r.ok) { showToast('Dodano', 'success'); showAddMemberModal.value = false; selectedUser.value = null; searchQuery.value = ''; addMemberRole.value = 'MEMBER'; refresh() }
    else { const e = await r.json(); showToast(e.error || 'Błąd', 'error') }
  } catch { showToast('Błąd połączenia', 'error') }
...
function confirmRemove(m: Member) { memberToRemove.value = m; showRemoveConfirm.value = true }
async function removeMember() {
  if (!memberToRemove.value) return; removingMember.value = true
  try {
    const r = await fetch(`${apiBase}/teams/${teamId}/members/${memberToRemove.value.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${useCookie('token').value || ''}` } })
    if (r.ok) { showToast('Usunięto', 'success'); showRemoveConfirm.value = false; memberToRemove.value = null; refresh() }
    else { const e = await r.json(); showToast(e.error || 'Błąd', 'error') }
  } catch { showToast('Błąd połączenia', 'error') }
  removingMember.value = false
}
async function changeRole(member: Member, newRole: string) {
  if (newRole === 'CAPTAIN' && !confirm('Przekazać kapitana?')) return
  try {
    const r = await fetch(`${apiBase}/teams/${teamId}/members/${member.id}/role`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${useCookie('token').value || ''}` }, body: JSON.stringify({ role: newRole }) })
    if (r.ok) { showToast('Rola zmieniona', 'success'); refresh() }
    else { const e = await r.json(); showToast(e.error || 'Błąd', 'error') }
  } catch { showToast('Błąd połączenia', 'error') }
}
async function leaveTeam() {
  if (!myMembership.value) return; if (!confirm('Opuścić drużynę?')) return
  try {
    const r = await fetch(`${apiBase}/teams/${teamId}/members/${myMembership.value.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${useCookie('token').value || ''}` } })
    if (r.ok) { showToast('Opuściłeś drużynę', 'success'); navigateTo('/teams') }
    else { const e = await r.json(); showToast(e.error || 'Błąd', 'error') }
  } catch { showToast('Błąd połączenia', 'error') }
}
async function deleteTeam() {
  if (!confirm('Czy na pewno chcesz usunąć tę drużynę? Tej operacji nie można cofnąć.')) return
  try {
    const r = await fetch(`${apiBase}/teams/${teamId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${useCookie('token').value || ''}` } })
    if (r.ok) { showToast('Drużyna usunięta', 'success'); navigateTo('/teams') }
    else { const e = await r.json(); showToast(e.error || 'Błąd', 'error') }
  } catch { showToast('Błąd połączenia', 'error') }
}
...
async function toggleRecruitment() {
  ...
  try {
    const ns = !team.value?.recruitment_open
    const r = await fetch(`${apiBase}/teams/${teamId}/recruitment`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${useCookie('token').value || ''}` }, body: JSON.stringify({ open: ns }) })
    if (r.ok) { showToast(ns ? 'Otwarta' : 'Zamknięta', 'success'); refresh() }
  } catch { showToast('Błąd', 'error') }
  togglingRecruitment.value = false
}
async function loadApplications() {
  showApplications.value = true
  try { const r = await fetch(`${apiBase}/teams/${teamId}/applications`, { headers: { Authorization: `Bearer ${useCookie('token').value || ''}` } }); if (r.ok) { const d: { data: Application[] } = await r.json(); applications.value = d.data || [] } } catch { }
}
async function handleApplication(appId: string, status: string) {
  try {
    const r = await fetch(`${apiBase}/teams/${teamId}/applications/${appId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${useCookie('token').value || ''}` }, body: JSON.stringify({ status }) })
    if (r.ok) { showToast(status === 'ACCEPTED' ? 'Zaakceptowano' : 'Odrzucono', 'success'); loadApplications(); refresh() }
    else { const e = await r.json(); showToast(e.error || 'Błąd', 'error') }
  } catch { showToast('Błąd połączenia', 'error') }
}

interface ChatMessage { id: string; teamId: string; userId: string; userName: string; text: string; createdAt: string; userRole?: string }
const chatMessages = ref<ChatMessage[]>([]); const newMessage = ref(''); const sendingChat = ref(false)
...
function scheduleChatPoll() {
  ...
  setTimeout(async () => {
    ...
    try {
      const r = await fetch(`${apiBase}/teams/${teamId}/chat`, { headers: { Authorization: `Bearer ${token}` } })
      if (r.ok) {
        const d = await r.json()
        if (d?.data && Array.isArray(d.data)) { chatMessages.value = d.data; setTimeout(() => { if (chatMessagesEl.value) chatMessagesEl.value.scrollTop = chatMessagesEl.value.scrollHeight }, 50) }
      }
    } catch { }
    scheduleChatPoll()
  }, 5000)
...
watch([myMembership, isAdminViewing], ([member, isAdmin]) => {
  if (member || isAdmin) { chatIsPolling = true; fetchChatMessages(); scheduleChatPoll() }
  else { chatIsPolling = false; chatMessages.value = [] }
}, { immediate: true })
onUnmounted(() => { chatIsPolling = false })
...
async function sendChatMessage() {
  ...
  try {
    ...
    const r = await fetch(`${apiBase}/teams/${teamId}/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ text: newMessage.value.trim(), user_name: userName }) })
    if (r.ok) {
      newMessage.value = ''
      const fr = await fetch(`${apiBase}/teams/${teamId}/chat`, { headers: { Authorization: `Bearer ${useCookie('token').value}` } })
      if (fr.ok) { const d = await fr.json(); if (d?.data) { chatMessages.value = d.data; setTimeout(() => { if (chatMessagesEl.value) chatMessagesEl.value.scrollTop = chatMessagesEl.value.scrollHeight }, 50) } }
    }
  } catch { }
  sendingChat.value = false
}

function formatChatTime(iso: string): string { return new Date(iso).toLocaleTimeString('pl-PL', { hour: '2-digit', minute: '2-digit' }) }
